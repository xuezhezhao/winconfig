...
class add(unittest.TestCase):
    ...
    def test_addVirCamera(self):
        ''' 设备管理-新增虚拟探头'''
        res=Camera().addVirCameraLs(camearkey,str(counternum),datatype,virnumber,businesstype).json()['description']
        self.assertEqual(res,'数据保存成功')
    def test_getVirCameraLs(self):
        ''' 设备管理-查看虚拟探头'''
        res=Camera().getVirCameraLs(camearkey).json()['msg']
